...
const load = () => {
  if (!existsSync(DB_PATH)) return { users: [], posts: [], orders: [], midman_requests: [], verifications: [], transactions: [], notifications: [] };
  try { return JSON.parse(readFileSync(DB_PATH, 'utf8')); } catch { return { users: [], posts: [], orders: [], midman_requests: [], verifications: [], transactions: [], notifications: [] }; }
};
const save = (data) => writeFileSync(DB_PATH, JSON.stringify(data, null, 2));

let data = load();
let seq = Math.max(0, ...Object.values(data).flatMap((arr) => arr.map((r) => r.id || 0))) + 1;
const nextId = () => seq++;
const num = (v) => (v ? 1 : 0);
...
export const dbUsers = {
  ...
  create: (username, passwordHash, role = 'user') => {
    const u = { id: nextId(), username: username.toLowerCase(), password_hash: passwordHash, role, is_verified: 0, verification_status: 'not_started', created_at: new Date().toISOString() };
    data.users.push(u); save(data); return u;
  },
  ...
  setVerification: (username, { isVerified, verificationStatus, declineReason = '' }) => {
    const u = dbUsers.findByUsername(username);
    if (u) { u.is_verified = num(isVerified); u.verification_status = verificationStatus; u.decline_reason = declineReason; save(data); }
  }
};
